...
class MeetingsController extends AppController
{
           ...
           // $user = $this->Users->find()->order(['id' => 'ASC']);
              ...
              if(!empty($this->request->data['sendto'])) {
                  ...
                  $data['company_id'] = $use->company_id;
                   ...
                   $data['meeting_date'] = date('d-m-Y',strtotime($meeting->date));
                   $data['meeting_title'] = $meeting->title;
                    $data['meeting_location'] = $meeting->location;
                    $data['meeting_invite'] = $meeting->invite;
                    $data['meeting_agenda'] = $meeting->agenda;
                   $data['user_name'] = $use->name;
                   $data['first_name'] = $use->first_name;
                   $data['last_name'] = $use->last_name;
                   $data['contactno'] = $use->tel;
                   $data['fee'] = $rsvp_settings->fee;
                   
                  //pr($meeting);die;  
                $myMailer = (new MyMailer())->rsvp_self($data);
                   }
            ...
            $meeting = $this->Meetings->patchEntity($meeting, $this->request->data);
              ...
              if(!empty($this->request->data['sendto'])) {
                ...
                'mem_type' => 'Associated'
                   ...
                   }elseif($this->request->data['sendto']=="e-Member"){
                     $companies=  $this->Companies->find('')
                ->where([
                'mem_type' => 'e-member'
            ]);  
    ...
    public function pastMeetings(){
        ...
        $this->paginate = [
           'conditions' =>array('Meetings.date >'=> '2017-09-01',
                      'Meetings.status <>' => 'D') ,
            'order' => [
                'Meetings.date' => 'desc'
            ]
    ...
    public function pastMeetingsSep(){
    ...
    public function nextMeeting($meeting_id=null){
...
//pr($meeting_id);die;
        ...
        if ($this->request->is('post')) {
            ...
            //   $data = $this->request->data;
              $invoice_data = [
                ...
                'attendees_name' => $data['user_name'].' '.$data['last_name'],
     ...
     public function pastMeeting($meeting_id=null){
        $this->set('title', 'Past Meeting');
//pr($meeting_id);die;
        $this->loadModel('Users');
        $user = $this->Users->find()
            ->contain(['Companies'])
 ...
 public function pastMeetingSep($meeting_id=null){
        $this->set('title', 'Next Meeting');
//pr($meeting_id);die;
        $this->loadModel('Users');
        $user = $this->Users->find()
